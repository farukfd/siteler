...
| NADAS Veri Terminali (`/endeks` + `/analyze` + `/locations`) | **CANLI (bağlandı)** | `nadas/index.html` → `EMLAK_TENANT.tenant_key` dolu (ProX anahtarı). Terminal **üç ucu** birden kullanır: **(1) /endeks** → `m²/score/delta/ilan_sayisi/trend` (canlı şerit + skor halkası + grafik ucu); **(2) /prox/analyze** (POST, 100 m² brüt) → `range.min_m2/max_m2/strongest_m2 + spread + confidence + piyasa_yonu + karar_ozeti + risk_ozeti + pazarlik_payi + eksik_bilgi` (grafikteki DEĞER ARALIĞI bandı + "ProX ANALİZ" paneli); **(3) /locations/{iller,ilceler,mahalleler}** → 81 il · gerçek ilçe/mahalle seçicileri (cascade, cache; yoksa gömülü REGIONS'a düşer, "81 İL · GERÇEK"/"5 İL · TEMSİLÎ" rozeti). Kategori→API eşleme: `kategori(konut\|ticari\|arsa)+durum(satilik\|kiralik)`; bina→konut, turistik→ticari, -Kir→kiralik. Geçmiş ŞEKİL modellenmiş (dürüstçe "MODELLENMİŞ ŞEKİL"), uç gerçeğe sabit; demografik (DNA) katmanı analizin altında (temsilî etiketli). Veri yok/CORS engel → dürüstçe **TEMSİLÎ**. **GÜVENLİK:** anahtar istemci-tarafı görünür → salt-okunur + alan-adı kısıtlı olmalı, ideali sunucu-proxy arkasında. |
