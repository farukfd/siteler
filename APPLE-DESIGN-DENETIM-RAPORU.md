...
3. **Dalga C — çıkış animasyonları:** site başına ortak `.closing` deseni (gm 4 overlay ailesi, dn pageOverlay+modallar, ins 9 overlay + 3 modal). — ✅ **UYGULANDI** (9 Ağu 2026): ortak `_xxAnimKapat` helper'ı (matchMedia reduced-motion kapısı + animationend + ~340ms güvenlik ağı + çifte-çağrı koruması). gm base v70/app v90: mnav slideout + modal fadeDown + br-page/saas ayna keyframe'leri, 5 kapanış fonksiyonu animasyonlu. dn base v25/app v54: #pageOverlay povIn materialize girişi + povOut çıkışı; homeView SENKRON geri gelir (fade altından ana sayfa belirir). ins base v5/app-core v38: 9 tam-sayfa overlay `.closing` fade + pointer-events:none; 3 modal (teklif .modal + giris .gmodal inline) pdIn/insMdIn materialize giriş + ayna çıkış; 11 kapanış noktası `_insOvKapat`'a çevrildi. Üç sitede canlı DOM yaşam-döngüsü testi (closing→animationend→sınıf temizliği) doğrulandı.
4. **Dalga D — performans cilası:** bar'lar scaleX'e, pfTilt/vcard transition çakışmaları, will-change temizliği, blog manşet crossfade. — ✅ **UYGULANDI** (9 Ağu 2026): 7 bar ailesi (br-cmprow/br-hcard/br-mrow/ozc-row/quota/ek-bar/bz-hbars) + dn/ins ekspertiz sayfaları `width`→`transform:scaleX()` (transform-origin:left; 8 JS yazıcı + 2 statik-sayfa yazıcısı `_sclX` desenine çevrildi; küçük değerlere .04 görünürlük tabanı); `barc` dikey grafiğin ölü height-transition'ı silindi; `bz-stack` yan-yana segmentli olduğundan (yerleşimsel width) bilinçli kapsam dışı. pfTilt: pointerenter'da `transition:none` (1:1 imleç takibi), pointerleave'de .32s yumuşak dönüş. `#infoPage .rv*` kalıcı will-change kaldırıldı. blogManGo teleport→crossfade (reduced-motion'da anında). gm base v71 · app v91 · kurumsal-sayfalar v13. Canlıda dn ekspertiz barı computed `matrix(0.86,...)` hedef-değer kanıtıyla doğrulandı.
5. **Dalga E (ayrı karar):** px→rem tipografi göçü — geniş dokunuşlu, ayrıca planlanmalı. — ✅ **UYGULANDI** (9 Ağu 2026): 78 dosyada ~4.470 bildirim `Npx→(N/16)rem` (deterministik, varsayılan ayarda birebir): `font-size` (4031) + `font:` shorthand LH'li/LH'siz (109+24) + `font-size:clamp()` içleri (301) + px `line-height`(3)/`letter-spacing`(14). Kapsam: 3 sitenin tüm public CSS'leri + HTML inline stiller + şablon üreten public JS'ler + shared/ modüller (admin-markup/content-studio/theme bilinçli hariç). 18 varlık ?v= bump (site-içi normalize). Canlı kanıt: dn `.btn` kök 20px'te 14→17.5px, `h1` 33→41.25px (Dynamic Type ✓); gm+ins base.css'te kalan px font-size 0. Not: test pane'inde tek nav linkinin sabit görünmesi Chrome text-autosizing (vw=0) artefaktı — üretim koşulu değil. Media-query breakpoint'leri bilinçli px bırakıldı.
