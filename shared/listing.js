...
(function(){
  ...
  L.injectCSS=function(){
    ...
    +'.lst-loc{font-size:12.5px;color:'+MUT+';font-weight:600}'
    +'.lst-title{font-size:14.5px;line-height:1.25;margin:0;color:'+INK+';font-weight:700;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}'
    ...
    +'.lst-price{font-size:17px;font-weight:800;color:'+INK+';font-family:var(--num),var(--body),sans-serif;font-variant-numeric:tabular-nums;letter-spacing:-.01em;line-height:1.1}'
    ...
    +'.lstd-type{font-size:12px;font-weight:600;color:'+MUT+';margin-left:6px}'
    +'.lstd-title{font-size:clamp(24px,4vw,38px);line-height:1.15;margin:10px 0 6px;font-weight:800}'
    +'.lstd-loc{font-size:14.5px;color:'+MUT+';font-weight:600}'
    ...
    +'.lstd-cover.lstd-empty{display:grid;place-items:center;font-size:54px;opacity:.4}'
    ...
    +'.lstd-price-m{display:none;font-size:24px;font-weight:800;color:'+A+';margin:14px 0}'
    +'@media(max-width:860px){.lstd-price-m{display:block}}'
    +'.lstd-section{margin-top:26px}.lstd-section h3{font-size:16px;font-weight:800;margin:0 0 12px;color:'+INK+'}'
    ...
    +'.lstd-spec .k{font-size:11.5px;color:'+MUT+';font-weight:600}.lstd-spec .v{font-size:15.5px;font-weight:800;color:'+INK+'}'
    +'.lstd-desc{font-size:15px;line-height:1.75}.lstd-desc p{margin:0 0 14px}.lstd-muted{color:'+MUT+'}'
    ...
    +'.lstd-feats li{position:relative;padding-left:24px;font-size:14px;line-height:1.5}.lstd-feats li:before{content:"✓";position:absolute;left:0;top:0;color:'+A+';font-weight:800}'
    +'.lstd-map{aspect-ratio:16/9;border-radius:14px;overflow:hidden;border:1px solid '+LINE+';position:relative}'
    +'.lstd-map-cap{position:absolute;left:0;bottom:0;background:rgba(0,0,0,.6);color:#fff;font-size:12px;padding:5px 10px;z-index:500;pointer-events:none}'
    +'.lstd-map-el{width:100%;height:100%;background:#e8eef3;position:relative}'
    +'.lstd-map-el iframe{width:100%;height:100%;border:0}'
    +'.lstd-map-load{position:absolute;inset:0;display:grid;place-items:center;color:'+MUT+';font-size:13px}'
    ...
    +'.lstd-map-guard button{background:rgba(0,0,0,.72);color:#fff;border:0;border-radius:999px;padding:9px 16px;font-size:12.5px;font-weight:700;cursor:pointer;pointer-events:none}'
    ...
    +'.lstd-price{font-size:26px;font-weight:800;color:'+A+';margin-bottom:10px}'
    +'.lstd-eids-row{margin-bottom:10px}'
    +'.lstd-brand{font-size:13px;color:'+MUT+';font-weight:700;margin-bottom:12px}'
    +'.lstd-btn{display:block;width:100%;text-align:center;padding:12px 14px;border-radius:11px;font-size:14px;font-weight:700;margin-top:9px;border:1px solid '+LINE+';background:'+BG+';color:'+INK+';cursor:pointer;text-decoration:none;box-sizing:border-box}'
    ...
    +'.lstd-note{margin-top:14px;font-size:12px;line-height:1.6;color:'+MUT+';display:flex;gap:8px;align-items:flex-start}.lstd-note b{color:'+INK+'}'
    ...
    +'.lst-inc{width:100%;display:inline-flex;align-items:center;justify-content:center;gap:8px;padding:11px 12px;border-radius:10px;font-size:13.5px;font-weight:700;font-family:inherit;border:0;cursor:pointer;color:var(--on-accent,#fff);background:var(--grad-cta,'+A+');box-shadow:0 10px 22px -12px '+A+'}'
    ...
    +'.lstd-attrs{width:100%;border-collapse:collapse;font-size:14px}'
    ...
    +'.at-f label{display:block;font-size:12px;font-weight:600;color:'+MUT+';margin-bottom:4px}'
    ...
    +'.at-feats{margin-top:16px}.at-feats-h{font-size:13.5px;font-weight:800;margin-bottom:10px}.at-muted{font-weight:400;color:'+MUT+';font-size:12px}'
    +'.at-chipgrp{margin-bottom:12px}.at-chipgrp-l{display:block;font-size:11.5px;color:'+MUT+';font-weight:700;margin-bottom:6px}'
    +'.at-chips{display:flex;flex-wrap:wrap;gap:7px}'
    +'.at-chip{border:1px solid '+LINE+';background:'+BG+';color:'+INK+';border-radius:999px;padding:6px 13px;font-size:12.5px;cursor:pointer;transition:all .12s}'
    ...
    +'.lstd-ag-ini{position:absolute;inset:0;display:grid;place-items:center;color:var(--on-accent,#fff);font-weight:800;font-size:18px;z-index:0}'
    +'.lstd-ag-info{display:flex;flex-direction:column;gap:2px;min-width:0}.lstd-ag-info b{font-size:15px}'
    +'.lstd-ag-title{font-size:12.5px;color:'+MUT+'}.lstd-ag-exp{font-size:12px;color:'+A+';font-weight:700}'
    /* CTA ikonları + Ara */
    ...
    +'.lstd-share-l{font-size:12px;color:'+MUT+';font-weight:600;display:block;margin-bottom:8px}'
    ...
    +'.lstd-back{background:'+SURF+';color:'+INK+';border:1px solid '+LINE+';border-radius:10px;padding:9px 16px;font-size:13.5px;font-weight:700;cursor:pointer;margin-bottom:6px}'
    ...
    +'.lstd-ovbrand{font-weight:800;font-size:19px;color:'+A+';text-decoration:none;white-space:nowrap;cursor:pointer}'
    +'.lstd-ovlinks{display:flex;gap:20px;flex:1;margin-left:8px}'
    +'.lstd-ovlinks a{color:'+INK+';text-decoration:none;font-size:14px;font-weight:600;white-space:nowrap}.lstd-ovlinks a:hover{color:'+A+'}'
    +'.lstd-ovcta{display:flex;align-items:center;gap:8px;margin-left:auto}'
    +'.lstd-ovcta a{width:38px;height:38px;border-radius:50%;display:grid;place-items:center;background:'+BG+';border:1px solid '+LINE+';text-decoration:none;font-size:16px}'
    +'.lstd-ovx{width:38px;height:38px;border-radius:50%;border:1px solid '+LINE+';background:'+BG+';color:'+INK+';cursor:pointer;font-size:15px}.lstd-ovx:hover{background:'+A+';color:var(--on-accent,#fff);border-color:transparent}'
