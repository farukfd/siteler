...
(function(){
  ...
  function make(host,cfg){
    ...
    function paint(m){
      ...
      if(o){ var list=ozelList(cfg,m);
        ...
        o.innerHTML='<div class="me-oz-band"><div class="me-oz-h"><span class="me-oz-lock">🔒 Özel Portföy · '+esc(m.mah)+'</span>'
          +'<p><b>Fiyatı görün, mülkünüzü ifşa etmeyin.</b> Bu mahalle ve çevresinde <b>açık ilana koymadan</b>, tam konum/EİDS paylaşmadan sattığımız/kiraladığımız size özel mülkler. Aşağıdaki fiyatlardan başlar:</p></div>'
  ...
  function injectCSS(){
    ...
    +'.me-badge{display:inline-flex;align-items:center;gap:7px;font:800 12px/1 system-ui;letter-spacing:.03em;color:'+A+';background:color-mix(in srgb,'+A+' 12%,transparent);padding:8px 12px;border-radius:999px;text-transform:uppercase}'
    ...
    +'.me-sels label{font:700 11px/1 system-ui;color:var(--me-mut);display:flex;flex-direction:column;gap:4px;text-transform:uppercase;letter-spacing:.03em}'
    +'.me-sels select{border:1px solid var(--me-line);border-radius:10px;padding:9px 11px;font:600 14px/1 system-ui;color:var(--me-ink);background:var(--me-surf);cursor:pointer;min-width:150px}'
    +'.me-live{font:600 12px/1.3 system-ui;color:var(--me-mut);display:inline-flex;align-items:center;gap:7px}'
    ...
    +'.me-h-mah{font:800 15px/1.2 system-ui}.me-h-mah span{color:var(--me-mut);font-weight:600}'
    +'.me-h-big{margin:6px 0 8px}.me-h-big b{font:800 30px/1 system-ui;color:'+A+'}.me-h-big span{color:var(--me-mut);font:600 13px/1 system-ui;margin-left:8px}'
    +'.me-h-meta{display:flex;gap:14px;flex-wrap:wrap;align-items:center;font:600 13px/1 system-ui}'
    +'.me-chg.up{color:#16a34a}.me-chg.up.big{font-size:15px;font-weight:800}.me-chg i{font-style:normal;opacity:.7;font-weight:600}'
    ...
    +'.me-score-ring span{position:relative;font:800 22px/1 system-ui;color:'+A+'}'
    +'.me-h-r{display:flex;align-items:center;gap:10px}.me-h-r small{color:var(--me-mut);font:600 11px/1.3 system-ui}'
    ...
    +'.me-val-lab{font:800 13px/1 system-ui;color:'+A+'}'
    +'.me-val-items{display:flex;gap:20px;flex-wrap:wrap;margin-top:10px}'
    +'.me-val-i{display:flex;flex-direction:column;gap:3px}.me-val-i small{font:600 11px/1 system-ui;color:var(--me-mut);text-transform:uppercase;letter-spacing:.02em}.me-val-i b{font:800 15px/1 system-ui}'
    +'.me-sec-h{font:800 13px/1 system-ui;color:var(--me-mut);text-transform:uppercase;letter-spacing:.04em;margin:20px 0 12px}'
    ...
    +'.me-cc-top{display:flex;justify-content:space-between;align-items:center}.me-cc-lab{font:700 12px/1.2 system-ui;color:var(--me-mut)}'
    ...
    +'.me-cc-val{font:800 19px/1.1 system-ui;margin:8px 0 2px}.me-cc-val small{font-size:12px;color:var(--me-mut);font-weight:700}'
    +'.me-cc-unit{font:600 11px/1.3 system-ui;color:var(--me-mut)}'
    +'.me-spark{width:100%;height:34px;margin:6px 0}'
    +'.me-cc-foot{display:flex;justify-content:space-between;align-items:center;font:700 11.5px/1 system-ui}'
    ...
    +'.me-prof-head{font:800 14px/1 system-ui;margin-bottom:14px}.me-prof-head em{font-style:normal;color:var(--me-mut);font-weight:600;font-size:12px}'
    ...
    +'.me-pie{text-align:center}.me-pie-t{font:700 12px/1 system-ui;color:var(--me-mut);margin-bottom:8px}'
    +'.me-donut-svg{width:108px;height:108px}.me-donut-c{font:800 18px/1 system-ui;fill:var(--me-ink)}'
    ...
    +'.me-legend li{display:flex;align-items:center;gap:6px;font:600 11.5px/1.3 system-ui;color:var(--me-mut)}.me-legend b{margin-left:auto;color:var(--me-ink)}'
    ...
    +'.me-stat{text-align:center}.me-stat span{display:block;font:600 11px/1.3 system-ui;color:var(--me-mut)}.me-stat b{font:800 16px/1.2 system-ui;color:'+A+'}'
    /* özel portföy */
    ...
    +'.me-oz-lock{font:800 15px/1 system-ui;color:#fff}'
    +'.me-oz-h p{margin:8px 0 0;font:500 13px/1.6 system-ui;color:#c3d0e0;max-width:640px}.me-oz-h b{color:#fff}'
    +'.me-oz-chips{display:flex;gap:8px;flex-wrap:wrap;margin:14px 0}'
    +'.me-oz-chip{background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.14);border-radius:999px;padding:7px 13px;font:600 12px/1 system-ui;color:#dbe6f5}.me-oz-chip b{color:#fff;font-weight:800}'
    ...
    +'.me-oz-op{display:inline-block;font:800 10px/1 system-ui;padding:4px 8px;border-radius:6px;text-transform:uppercase}.me-oz-op.s{background:'+A+';color:#fff}.me-oz-op.k{background:#c99a2e;color:#1a1206}'
    +'.me-oz-tip{font:700 13px/1 system-ui;color:#fff;margin:8px 0 4px}.me-oz-loc{font:600 12px/1.3 system-ui;color:#aebfd2}'
    +'.me-oz-tech{display:flex;gap:6px;margin:8px 0}.me-oz-tech span{background:rgba(255,255,255,.08);border-radius:6px;padding:3px 8px;font:600 11px/1 system-ui;color:#cdd9e8}'
    +'.me-oz-price{font:800 17px/1 system-ui;color:#fff;margin:6px 0 10px}.me-oz-price small{display:block;font-size:10.5px;color:#9fb0c4;font-weight:600;margin-top:3px}'
    +'.me-oz-cta{display:flex;gap:6px}'
    +'.me-oz-detail{flex:1;border:0;background:'+A+';color:#fff;border-radius:9px;padding:9px;font:700 12px/1 system-ui;cursor:pointer}'
    ...
    +'.me-oz-all{border:0;background:#fff;color:#12181f;border-radius:10px;padding:11px 18px;font:800 13px/1 system-ui;cursor:pointer}'
    +'.me-oz-owner{display:inline-flex;align-items:center;gap:6px;border:1px solid rgba(255,255,255,.25);color:#fff;border-radius:10px;padding:11px 16px;font:700 13px/1 system-ui;text-decoration:none}.me-oz-owner:hover{background:rgba(255,255,255,.1)}'
    /* sokaklar */
    ...
    +'.me-st-name{font:600 12.5px/1.3 system-ui;color:var(--me-mut)}.me-st-name b{color:var(--me-ink)}'
    +'.me-st-tech{display:flex;gap:6px;margin:8px 0}.me-st-tech span{background:color-mix(in srgb,'+A+' 8%,transparent);border-radius:6px;padding:3px 7px;font:600 11px/1 system-ui;color:var(--me-ink)}'
