...
(function(){
  ...
  L._extPrint=function(){ try{ var l=L._cur, cfg=L._curCfg||{}; if(!l)return;
    ...
    var html='<!doctype html><html lang="tr"><head><meta charset="utf-8"><title>'+esc(l.title||'İlan')+'</title>'
      +'<style>*{box-sizing:border-box}body{font:14px/1.5 -apple-system,system-ui,sans-serif;color:#111;margin:0;padding:28px}'
      +'h1{font-size:22px;margin:0 0 4px}.loc{color:#555;margin-bottom:8px}.price{font-size:24px;font-weight:800;color:#0e7c86;margin:8px 0 16px}'
      ...
      +'.brand{margin-top:20px;padding-top:12px;border-top:2px solid #0e7c86;font-weight:700}.eids{margin-top:8px;font-size:12px;color:#16a34a}.desc{margin:10px 0}h3{font-size:15px;margin:16px 0 6px}@media print{body{padding:0}}</style></head><body>'
      +'<h1>'+esc(l.title||'')+'</h1><div class="loc">📍 '+esc([l.mah,l.ilce,l.il].filter(Boolean).join(', '))+'</div>'
      +(im?'<img src="'+esc(im)+'">':'')
      +'<div class="price">'+esc(l.priceText||l.price||'')+' <span style="font-size:14px;color:#555">· '+esc(l.op||'')+' · '+esc(l.type||'')+'</span></div>'
  ...
  (function(){
    ...
    +'.lstd-mbtn{display:inline-flex;align-items:center;gap:7px;border:1px solid '+LINE+';background:'+SURF+';color:'+INK+';border-radius:11px;padding:10px 15px;font:700 13.5px/1 system-ui;cursor:pointer;transition:.15s}'
    +'.lstd-mbtn:hover{border-color:'+A+';color:'+A+'}'
    +'.lstd-mbtn.on{background:'+A+';color:#fff;border-color:'+A+'}'
    +'.lstd-mbtn-ic{font-size:15px}'
    ...
    +'.lstd-stage-in.lstd-empty{display:grid;place-items:center;font-size:48px;color:rgba(255,255,255,.3)}'
    ...
    +'.lstd-gcount{background:rgba(0,0,0,.6);color:#fff;border-radius:7px;font:700 11px/1 system-ui;padding:5px 9px}'
    ...
    +'.lstd-th-ph{display:grid;place-items:center;width:100%;height:100%;font-size:24px;color:#fff}'
    +'.lstd-th-badge{position:absolute;bottom:4px;right:4px;background:rgba(0,0,0,.75);color:#fff;border-radius:5px;font:800 9px/1 system-ui;padding:3px 5px}'
    ...
    +'.lstd-mload{color:#fff;opacity:.85;text-align:center;padding:40px 20px;font:600 14px/1.5 system-ui;margin:auto}'
    +'.lstd-mhint{background:rgba(255,255,255,.06);color:#cbd5e1;font:500 12px/1.4 system-ui;padding:8px 12px;text-align:center}'
    +'.lstd-cover{position:relative}.lstd-zoom{position:absolute;right:10px;bottom:10px;background:rgba(0,0,0,.55);color:#fff;border-radius:8px;padding:5px 9px;font-size:14px;pointer-events:none;z-index:2}'
    /* meta */
    +'.lstd-meta{display:flex;gap:16px;flex-wrap:wrap;margin-top:8px;color:'+MUT+';font:500 12.5px/1.4 system-ui}'
    ...
    +'.lstd-energy-l{font:600 12.5px/1 system-ui;color:'+MUT+'}'
    +'.lstd-energy-b{display:inline-grid;place-items:center;min-width:30px;height:26px;padding:0 8px;border-radius:7px;color:#fff;font:800 15px/1 system-ui}'
    /* finans */
    +'.lstd-fin-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}'
    +'.lstd-fin label{display:flex;flex-direction:column;gap:5px;font:600 12.5px/1.2 system-ui;color:'+MUT+'}'
    +'.lstd-fin input,.lstd-fin select{border:1px solid '+LINE+';border-radius:9px;padding:9px 10px;font:600 14px/1 system-ui;color:'+INK+';background:'+SURF+'}'
    ...
    +'.lstd-fin-hero-ic{font-size:22px;position:relative}'
    +'.lstd-fin-hero-l{font:700 12px/1 system-ui;letter-spacing:.04em;text-transform:uppercase;opacity:.92;position:relative;margin-top:4px}'
    +'.lstd-fin-hero-v{font:800 clamp(28px,7vw,40px)/1 system-ui;letter-spacing:-.5px;margin:9px 0 6px;position:relative}'
    +'.lstd-fin-hero-sub{font:600 12px/1.4 system-ui;opacity:.9;position:relative}'
    ...
    +'.lstd-fin-chip span{color:'+MUT+';font:600 12.5px/1.2 system-ui}'
    +'.lstd-fin-chip b{color:'+INK+';font:800 14px/1 system-ui}'
    +'.lstd-fin-note{margin-top:10px;color:'+MUT+';font:500 11.5px/1.5 system-ui}'
    /* benzer */
    ...
    +'.lst-lb-x{position:absolute;top:16px;right:18px;width:44px;height:44px;border-radius:50%;border:0;background:rgba(255,255,255,.14);color:#fff;font-size:22px;cursor:pointer}'
    +'.lst-lb-nav{position:absolute;top:50%;transform:translateY(-50%);width:52px;height:52px;border-radius:50%;border:0;background:rgba(255,255,255,.14);color:#fff;font-size:34px;line-height:1;cursor:pointer}'
    +'.lst-lb-nav.prev{left:16px}.lst-lb-nav.next{right:16px}'
    +'.lst-lb-count{position:absolute;bottom:20px;left:50%;transform:translateX(-50%);color:#fff;background:rgba(0,0,0,.5);border-radius:20px;padding:6px 16px;font:600 13px/1 system-ui}'
    /* karşılaştırma */
    ...
    +'.lst-cmpbar-n{font:600 13.5px/1 system-ui}.lst-cmpbar-n b{color:#fff}'
    +'.lst-cmpbar-go{border:0;background:'+A+';color:#fff;border-radius:9px;padding:9px 15px;font:700 13px/1 system-ui;cursor:pointer}'
    +'.lst-cmpbar-x{border:0;background:rgba(255,255,255,.15);color:#fff;border-radius:8px;width:32px;height:32px;font-size:15px;cursor:pointer}'
    ...
    +'.lst-cmp-head h3{margin:0;font:800 18px/1 system-ui;color:'+INK+'}'
    +'.lst-cmp-x{border:0;background:rgba(0,0,0,.06);border-radius:8px;width:34px;height:34px;font-size:16px;cursor:pointer;color:'+INK+'}'
    +'.lst-cmp-scroll{overflow-x:auto}'
    +'.lst-cmp-tbl{border-collapse:collapse;width:100%;min-width:520px}'
    +'.lst-cmp-tbl th{position:sticky;left:0;background:'+SURF+';text-align:left;color:'+MUT+';font:700 12.5px/1.3 system-ui;padding:10px 14px;border-bottom:1px solid '+LINE+';white-space:nowrap}'
    +'.lst-cmp-tbl td{padding:10px 14px;border-bottom:1px solid '+LINE+';border-left:1px solid '+LINE+';font:500 13px/1.4 system-ui;color:'+INK+';vertical-align:top;min-width:150px}'
    +'.lst-cmp-open{border:0;background:'+A+';color:#fff;border-radius:8px;padding:7px 12px;font:700 12px/1 system-ui;cursor:pointer}'
    ...
    +'.lst-mf-h{font:800 14px/1.3 system-ui;color:'+INK+';margin-bottom:12px}.lst-mf-h span{font-weight:500;color:'+MUT+';font-size:12px}'
    +'.lst-mf-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}'
    +'.lst-mf-cell{display:flex;flex-direction:column;gap:5px;font:600 12.5px/1.3 system-ui;color:'+INK+'}'
    +'.lst-mf-cell i{font-weight:500;color:'+MUT+';font-style:normal}'
    +'.lst-mf-cell input,.lst-mf-cell select{border:1px solid '+LINE+';border-radius:9px;padding:9px 10px;font:500 13px/1.2 system-ui;background:'+SURF+';color:'+INK+'}'
    +'.lst-mf-ok{color:#16a34a;font:700 11.5px/1 system-ui;font-style:normal}'
    +'.lst-mf-note{margin-top:10px;color:'+MUT+';font:500 11.5px/1.5 system-ui}'
