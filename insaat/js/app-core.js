...
window.addEventListener("error",e=>{console.warn("[caught]",e.message,e.filename+":"+e.lineno);e.preventDefault&&e.preventDefault();return true;});
/* ---------------- DATA ---------------- */
const IMG = {"h0": "img/insaat/img1.jpg", "h1": "img/insaat/img2.jpg", "h2": "img/insaat/img3.jpg", "h3": "img/insaat/img4.jpg", "p_res": "img/insaat/img5.jpg", "p_lux": "img/insaat/img6.jpg", "p_villa": "img/insaat/img7.jpg", "p_office": "img/insaat/img8.jpg", "p_white": "img/insaat/img9.jpg", "p_home": "img/insaat/img10.jpg", "about": "img/insaat/img11.jpg", "b1": "img/insaat/img12.jpg", "b2": "img/insaat/img13.jpg", "b3": "img/insaat/img14.jpg"};
...
const BLOG=[
  {img:'b1',date:'12 Haziran 2026',t:'Kat karşılığı inşaatta arsa sahibinin bilmesi gereken 7 madde',d:'Hisse oranından teslim süresine, sözleşmede kritik başlıklar.'},
  {img:'b2',date:'28 Mayıs 2026',t:'Deprem güvenli bina nasıl anlaşılır? TBDY 2018 rehberi',d:'Zemin etüdü, taşıyıcı sistem ve denetim kriterleri.'},
  {img:'b3',date:'9 Mayıs 2026',t:'Tadilatta bütçe nasıl yönetilir? Adım adım rehber',d:'Keşiften malzeme seçimine, sürprizsiz tadilatın yolu.'},
];
let LEADS=[];
...
let MENU={ hizmetler:'Hizmetlerimiz', nedenBiz:'Neden ? Biz', projeler:'Projeler', bolge:'Bölge Zekası', giris:'Giriş', teklif:'Ücretsiz Keşif' };
...
const MENU_EN={hizmetler:'Services',nedenBiz:'Why ? Us',projeler:'Projects',bolge:'Regional Intelligence',giris:'Sign In',teklif:'Free Survey'};
...
function applyMenuText(){
  try{
    ...
    document.querySelectorAll('.insaatNav').forEach(function(n){var a=n.querySelectorAll('a');if(a[0])a[0].textContent=M.hizmetler;if(a[1])a[1].innerHTML=nz(M.nedenBiz);if(a[2])a[2].textContent=M.projeler;if(a[3])a[3].textContent=M.bolge;});
    ...
    document.querySelectorAll('.insaatMnav').forEach(function(m){var a=m.querySelectorAll('a');if(a[0])a[0].textContent=M.hizmetler;if(a[1])a[1].innerHTML=nz(M.nedenBiz);if(a[2])a[2].textContent=M.projeler;if(a[3])a[3].textContent=M.bolge;if(a[4])a[4].textContent=M.giris;var b=m.querySelector('.btn-primary');if(b)b.textContent=M.teklif;});
    document.querySelectorAll('.insaatFooter').forEach(function(f){
      ...
      if(cols[2]){var _hh=en?['Residential Construction','Renovation & Refurbishment','Urban Renewal','Land-for-Flat','Turnkey','Commercial Buildings']:['Konut İnşaatı','Tadilat & Renovasyon','Kentsel Dönüşüm','Kat Karşılığı','Anahtar Teslim','Ticari Yapılar'];cols[2].querySelectorAll('a').forEach(function(a,i){if(_hh[i])a.textContent=_hh[i];});}
...
const INSAAT_FOOTER=`<div class="wrap">
  <div class="fgrid">
    <div><div class="lo">Meridyen<span class="lo2"> Yapı</span></div>
      ...
      <div class="flegal"><a href="#" onclick="closeAllInsaatOverlays();openDoc('kvkk');return false">KVKK</a><a href="#" onclick="closeAllInsaatOverlays();openDoc('gizlilik');return false">Gizlilik</a><a href="#" onclick="closeAllInsaatOverlays();openDoc('cerez');return false">Çerez Politikası</a></div>
    </div>
    <div><h4>Kurumsal</h4><a href="neden-biz.html">Hakkımızda</a><a href="#" onclick="closeAllInsaatOverlays();openDoc('vizyon');return false">Vizyon & Misyon</a><a href="#" onclick="closeAllInsaatOverlays();openDoc('yonetim');return false">Yönetim Kadrosu</a><a href="#" onclick="closeAllInsaatOverlays();openDoc('kalite');return false">Kalite & Sertifikalar</a><a href="#medya" onclick="return fGo('medya')">Medya & Basında Biz</a><a href="#bolge" onclick="closeAllInsaatOverlays();openBolgePage();return false">Bölge Ekspertizi</a><a href="#" onclick="closeAllInsaatOverlays();openDoc('kariyer');return false">Kariyer</a><a href="#sss" onclick="closeAllInsaatOverlays();openFaqPage();return false">İnşaata Başlamadan Dikkat Edilecekler</a></div>
    <div><h4>Hizmetler</h4><a href="#hizmetler" onclick="closeAllInsaatOverlays();openHizmetlerPage();return false">Konut İnşaatı</a><a href="#hizmetler" onclick="closeAllInsaatOverlays();openHizmetlerPage();return false">Tadilat & Renovasyon</a><a href="#hizmetler" onclick="closeAllInsaatOverlays();openHizmetlerPage();return false">Kentsel Dönüşüm</a><a href="#hizmetler" onclick="closeAllInsaatOverlays();openHizmetlerPage();return false">Kat Karşılığı</a><a href="#hizmetler" onclick="closeAllInsaatOverlays();openHizmetlerPage();return false">Anahtar Teslim</a><a href="#hizmetler" onclick="closeAllInsaatOverlays();openHizmetlerPage();return false">Ticari Yapılar</a></div>
    <div><h4>İletişim</h4>
  ...
  <div class="fbot">
    <div class="fcopy">© 2026 Meridyen Yapı A.Ş. · Tüm hakları saklıdır. · <span style="opacity:.7">Kurgusal tanıtım demosu.</span> · <span class="fvo">🔗 emlakekspertizi.com Veri Ortağı</span></div>
...
function fetchSaaSData(){return new Promise(r=>setTimeout(r,120)).then(()=>{
  ...
  console.log('[SaaS API] '+SAAS_CONFIG.tenantName+' · yetki: '+(sc.ilceler&&sc.ilceler.length?sc.ilceler.join(', '):'Tüm bölgeler')+' → '+projects.length+' proje · '+lands.length+' arsa');
...
async function proxAiQuery(userMessage,contextType){const ctx=contextType||'default';
  ...
  try{
    if(typeof proxApi==='function'){
      /* Canlı /prox/ai body: {prompt}. Persona/sistem promptu metne katlanır. */
...
async function saasAutoThemeFromLogo(logoUrl){await new Promise(r=>setTimeout(r,150));const u=(logoUrl||'').toLocaleLowerCase('tr');let theme='Turuncu';
  ...
  SAAS_CONFIG.tenantSettings.logoUrl=logoUrl;SAAS_CONFIG.tenantSettings.themeColor=theme;initSaaSTheme();console.log('[Logo Adaptasyon] '+logoUrl+' → tema: '+theme);return theme;}
...
async function saasGenerateReportPDF(propertyId){
  ...
  if(a){ /* değerler API'den */
    ...
    console.log('Proje Analizi (API): '+propertyId+' · değer='+a.strongest_value+' · güven=%'+a.confidence);
  } else {
    console.log('Proje analizi: canlı veri yok, yerel bölge bağlamı kullanıldı: '+propertyId);
  }
...
function renderBlog(){document.getElementById('blogGrid').innerHTML=BLOG.map(b=>{const s=imgFor(b.img);
...
function fmtTL(n){return n>=1e6?(n/1e6).toFixed(1)+'M ₺':n.toLocaleString('tr-TR')+' ₺';}
function kkHesap(a){
  const insaatAlani=a.m2*(a.imar.emsal||0);
  const tabanAlani=a.m2*(a.imar.taks||0);
  const kk=a.kk||{};
