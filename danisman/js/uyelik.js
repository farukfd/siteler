...
(function () {
  ...
  function _injectCSS(){
    ...
    +'.gu-x{position:absolute;top:13px;right:15px;background:none;border:0;color:'+MUT+';font-size:21px;cursor:pointer;line-height:1}'
    +'.gu-head{display:flex;align-items:center;gap:11px;font-weight:700;font-size:16.5px;margin-bottom:18px;font-family:'+SERIF+'}.gu-head .mark{width:34px;height:34px;border-radius:9px;background:'+GOLD+';color:'+GOLDTX+';display:grid;place-items:center;font-weight:800;font-family:'+SERIF+';flex:0 0 auto}'
    +'.gu-tabs{display:flex;gap:8px;margin-bottom:16px}.gu-tabs button{flex:1;background:'+SURF2+';border:1px solid '+LINE+';color:'+MUT+';border-radius:11px;padding:10px;font-weight:600;font-size:13.5px;cursor:pointer;transition:.16s;font-family:'+HEAD+'}.gu-tabs button.act{background:'+EM+';color:#f3efe2;border-color:transparent;box-shadow:0 8px 20px -10px rgba(14,94,62,.6)}'
    +'.gu-pane label{display:block;font-size:12.5px;color:'+MUT+';margin:12px 0 5px;font-weight:600}'
    +'.gu-pane input[type=email],.gu-pane input[type=password],.gu-pane input:not([type]){width:100%;background:'+BG+';border:1px solid '+LINE+';border-radius:11px;padding:12px 14px;color:'+INK+';font-size:14px;box-sizing:border-box;font-family:'+HEAD+'}'
    +'.gu-pane input:focus{outline:none;border-color:var(--em-2,#14805a);box-shadow:0 0 0 3px rgba(20,128,90,.12)}'
    +'.gu-btn{width:100%;margin-top:18px;background:'+GOLD+';color:'+GOLDTX+';border:0;border-radius:11px;padding:13px;font-weight:700;font-size:14px;cursor:pointer;font-family:'+HEAD+';box-shadow:0 16px 34px -18px rgba(163,126,45,.7)}.gu-btn:hover{filter:brightness(1.05)}'
    +'.gu-btn2{width:100%;margin-top:9px;background:transparent;color:'+INK+';border:1px solid '+LINE+';border-radius:11px;padding:12px;font-weight:700;font-size:13.5px;cursor:pointer;font-family:'+HEAD+'}.gu-btn2:hover{border-color:var(--em-2,#14805a);color:var(--em,#0e5e3e)}'
    +'.gu-btn-yon{background:'+EM+';color:#f3efe2;border:0;box-shadow:0 14px 30px -16px rgba(14,94,62,.6)}'
    +'.gu-err{min-height:16px;font-size:12.5px;margin-top:10px}.gu-err.bad{color:'+RED+'}.gu-err.ok{color:'+PROXD+'}'
    +'.gu-note{font-size:12.5px;color:'+MUT+';margin-top:14px;line-height:1.7}.gu-note a{color:var(--em-2,#14805a);text-decoration:none;font-weight:600}.gu-note a:hover{text-decoration:underline}'
    +'.gu-div{display:flex;align-items:center;gap:12px;margin:16px 0 4px;color:'+MUT+';font-size:11.5px;text-transform:uppercase;letter-spacing:.1em}.gu-div::before,.gu-div::after{content:"";flex:1;height:1px;background:'+LINE+'}'
    +'.gu-sub{font-size:13px;color:'+MUT+';margin:0 0 10px;line-height:1.6}'
    +'.gu-kvkk{display:flex;gap:9px;align-items:flex-start;margin-top:13px}.gu-kvkk label{margin:0;font-size:11.5px;line-height:1.5}.gu-kvkk input{margin-top:2px}'
    +'.gu-prof{display:flex;align-items:center;gap:14px}.gu-av{width:54px;height:54px;border-radius:50%;background:'+EM+';color:#f3efe2;display:grid;place-items:center;font-size:23px;font-weight:800;font-family:'+SERIF+'}.gu-pname{font-weight:700;font-size:16.5px;font-family:'+SERIF+'}.gu-pmail{color:'+MUT+';font-size:13px}'
    ...
    +'.dnh-logo{display:inline-flex;align-items:center;gap:10px;font-weight:700;font-size:18px;color:'+INK+';text-decoration:none;cursor:pointer;font-family:'+SERIF+'}.dnh-logo .mark{width:34px;height:34px;border-radius:9px;background:'+GOLD+';color:'+GOLDTX+';display:grid;place-items:center;font-weight:800}'
    +'.dnh-back{margin-left:auto;background:'+SURF2+';color:'+INK+';border:1px solid '+LINE+';border-radius:10px;padding:9px 16px;font-weight:600;font-size:13.5px;cursor:pointer;font-family:'+HEAD+'}.dnh-back:hover{border-color:var(--em-2,#14805a);color:var(--em,#0e5e3e)}'
    ...
    +'.hs-avatar{width:58px;height:58px;border-radius:50%;background:'+EM+';color:#f3efe2;display:grid;place-items:center;font-size:25px;font-weight:800;font-family:'+SERIF+'}'
    +'.hs-hi .h{font-size:19px;font-weight:700;font-family:'+SERIF+'}.hs-hi .h b{color:var(--em,#0e5e3e)}.hs-hi .s{color:'+MUT+';font-size:13px}'
    +'.hs-kpis{display:flex;gap:12px;margin-left:auto}.hs-kpi{background:'+BG+';border:1px solid '+LINE+';border-radius:12px;padding:11px 20px;text-align:center}.hs-kpi b{display:block;font-size:23px;font-family:'+SERIF+';color:var(--em,#0e5e3e)}.hs-kpi span{font-size:11.5px;color:'+MUT+'}'
    ...
    +'.hs-tabs{display:flex;gap:8px;margin:22px 0}.hs-tabs button{background:'+SURF+';border:1px solid '+LINE+';color:'+MUT+';border-radius:11px;padding:11px 20px;font-weight:600;font-size:14px;cursor:pointer;font-family:'+HEAD+'}.hs-tabs button.act{background:'+EM+';color:#f3efe2;border-color:transparent}'
    +'.hs-grid{display:grid;grid-template-columns:1fr 1fr;gap:16px}@media(max-width:760px){.hs-grid{grid-template-columns:1fr}}'
    +'.hs-card{background:'+SURF+';border:1px solid '+LINE+';border-radius:16px;padding:22px 24px}.hs-card h3{margin:0 0 14px;font-size:16.5px;font-family:'+SERIF+'}'
    +'.hs-card label{display:block;font-size:12.5px;color:'+MUT+';margin:12px 0 5px;font-weight:600}'
    +'.hs-card input,.hs-card select,.hs-card textarea{width:100%;background:'+BG+';border:1px solid '+LINE+';border-radius:11px;padding:12px 14px;color:'+INK+';font-size:14px;box-sizing:border-box;font-family:'+HEAD+'}'
    ...
    +'.hs-ro{background:'+BG+';border:1px solid '+LINE+';border-radius:11px;padding:12px 14px;color:'+MUT+';font-size:14px}'
    ...
    +'.hs-msg{min-height:16px;font-size:12.5px;margin-top:10px}.hs-msg.bad{color:'+RED+'}.hs-msg.ok{color:'+PROXD+'}'
    +'.hs-quickq h3{margin-bottom:6px}.hs-qnote{font-size:11.5px;color:'+MUT+';margin-top:11px;line-height:1.55}'
    +'.hs-qlist{display:flex;flex-direction:column;gap:12px;margin-top:16px}'
    +'.hs-quote{background:'+SURF+';border:1px solid '+LINE+';border-radius:14px;padding:17px 19px}.hs-quote .qhead{display:flex;justify-content:space-between;gap:10px;align-items:flex-start}.hs-quote .qk{font-weight:700;font-size:15px}.hs-quote .qd{color:'+MUT+';font-size:12px;margin-top:2px}'
    +'.qbadge{flex:0 0 auto;font-size:12px;font-weight:700;padding:5px 12px;border-radius:999px}.qbadge.ok{background:rgba(25,195,125,.16);color:'+PROXD+'}.qbadge.wait{background:rgba(195,155,69,.16);color:var(--gold-deep,#a17e2d)}'
    +'.hs-quote .qmsg{color:'+MUT+';font-size:13.5px;font-style:italic;margin:11px 0 0}'
    +'.hs-quote .qcevap{margin-top:13px;background:'+SURF2+';border-left:3px solid '+PROX+';border-radius:0 11px 11px 0;padding:12px 15px}.hs-quote .qcevap b{font-size:12px;color:'+PROXD+'}.hs-quote .qcevap p{margin:5px 0 0;font-size:14px;line-height:1.65}'
    +'.hs-quote .qpending{margin-top:13px;color:'+MUT+';font-size:13px;display:flex;align-items:center;gap:9px}'
    +'.hs-empty{color:'+MUT+';font-size:14px;padding:24px;text-align:center}'
    /* ---- Admin üye bloğu ---- */
    +'.dnu-wrap{margin-top:8px}.dnu-quotes{display:flex;flex-direction:column;gap:10px}'
    +'.dnu-q{background:var(--panel,#fff);border:1px solid '+LINE+';border-radius:11px;padding:13px 15px}.dnu-qh{display:flex;align-items:center;justify-content:space-between;gap:10px}.dnu-qh b{font-size:14px}.dnu-badge{flex:0 0 auto;font-size:11px;font-weight:800;padding:4px 10px;border-radius:999px}.dnu-badge.ok{background:rgba(25,195,125,.16);color:'+PROXD+'}.dnu-badge.wait{background:rgba(195,155,69,.16);color:var(--gold-deep,#a17e2d)}'
    +'.dnu-qm{color:'+MUT+';font-size:12px;margin-top:5px}.dnu-qt{font-size:13.5px;font-style:italic;margin-top:8px}.dnu-qc{margin-top:9px;background:var(--panel-2,#f7f3ea);border-left:3px solid '+PROX+';border-radius:0 9px 9px 0;padding:9px 12px;font-size:13px;line-height:1.55}.dnu-qc b{color:'+PROXD+';font-size:11.5px}'
    +'.dnu-tblwrap{overflow-x:auto;border:1px solid '+LINE+';border-radius:11px}.dnu-tbl{width:100%;border-collapse:collapse;font-size:13.5px}.dnu-tbl th,.dnu-tbl td{text-align:left;padding:11px 14px;border-bottom:1px solid '+LINE+'}.dnu-tbl th{color:'+MUT+';font-size:11.5px;text-transform:uppercase;letter-spacing:.03em}.dnu-tbl tr:last-child td{border-bottom:0}'
    /* ---- hesap açıkken ana header GİZLE (z-index çakışması kesin fix) ---- */
    +'html.dn-hide-hdr header.nav{display:none!important}'
    /* ---- Favorilerim (hesap) ---- */
    +'.fav-sec{margin-bottom:26px}.fav-sech{font-family:'+SERIF+';font-size:17px;margin:0 0 12px;display:flex;align-items:center;gap:9px;color:'+INK+'}.fav-sech span{font-family:'+HEAD+';font-size:12px;font-weight:700;background:'+SURF2+';color:'+MUT+';border-radius:999px;padding:2px 10px}'
    ...
    +'.fav-cbody{min-width:0;flex:1}.fav-ct{font-weight:700;font-size:15px;line-height:1.3}.fav-tag{font-family:'+HEAD+';font-size:10.5px;font-weight:700;text-transform:uppercase;letter-spacing:.05em;color:'+GOLDTX+';background:'+GOLD+';border-radius:5px;padding:2px 7px;margin-left:5px;vertical-align:middle}'
    +'.fav-cl{color:'+MUT+';font-size:12.5px;margin:5px 0}.fav-cp{color:var(--em,#0e5e3e);font-weight:800;font-size:15px}.fav-cp span{font-weight:500;font-size:11.5px;color:'+MUT+'}.fav-cs{color:'+MUT+';font-size:12.5px;line-height:1.5;margin-top:5px}'
    +'.fav-cact{display:flex;flex-direction:column;gap:8px;align-items:flex-end;flex:0 0 auto}.fav-go{background:none;border:0;color:var(--em-2,#14805a);font-weight:700;font-size:13px;cursor:pointer;white-space:nowrap;padding:0}.fav-go:hover{text-decoration:underline}'
    +'.fav-rm{width:34px;height:34px;border-radius:50%;background:'+SURF2+';border:1px solid '+LINE+';color:#c0392b;cursor:pointer;font-size:16px;line-height:1}.fav-rm:hover{border-color:#c0392b}'
    +'.fav-empty{color:'+MUT+';font-size:13px;padding:10px 2px}';
