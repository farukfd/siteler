...
(function () {
  ...
  var PA_GREET='Merhaba, ben Meridyen Gayrimenkul ProX Asistanı 👋 Size nasıl yardımcı olabilirim? Satılık ya da kiralık mı arıyorsunuz, mülkünüzü mü değerlendirmek istiyorsunuz, yoksa değerleme mi? Sizi dinliyorum — doğru gayrimenkule birlikte ulaşalım. Yanıtlarım, Türkiye’nin kapsamlı emlak veritabanı ProX’un 480 milyon+ doğrulanmış veri noktasına dayanır.';
  ...
  /* AI persona marka-ikame: DOM sweep sistem-prompt'una ulaşamaz → modele giden metinde brandName() uygula (sızıntı F) */
  function _paBS(s){var b=(typeof brandName==='function'&&brandName())||'Meridyen Gayrimenkul';var sh=(b.split(/\s+/)[0])||b;return (''+s).split('Meridyen Gayrimenkul').join(b).split('Meridyen').join(sh);}
  function _paPrompt(){var b=_paBizContext();return _paBS(PA_SYS)+(b?('\n\n'+b):'');}
