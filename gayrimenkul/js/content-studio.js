...
(function () {
  ...
  /* ---------- PUBLIC RENDER CSS (mount olmadan da /blog detayında video+galeri düzgün) ---------- */
  (function _pubCss(){try{if(document.getElementById('cs-pub-css'))return;
    var css='.cs-video{margin:22px 0}.cs-video-frame{position:relative;width:100%;aspect-ratio:16/9;border-radius:12px;overflow:hidden;background:#000}.cs-video-frame iframe,.cs-video-frame video{position:absolute;inset:0;width:100%;height:100%;border:0;display:block}.cs-video figcaption{font-size:12.5px;opacity:.7;margin-top:6px;text-align:center}.cs-gal{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:14px;margin:18px 0}.cs-gal>figure{margin:0}.cs-gal img{width:100%;border-radius:12px;display:block}.cs-gal figcaption,.cs-gal-cap{font-size:12px;opacity:.7;margin-top:4px}.cs-gal-cap{grid-column:1/-1;text-align:center}';
    var st=document.createElement('style');st.id='cs-pub-css';st.textContent=css;(document.head||document.documentElement).appendChild(st);
  }catch(e){}})();

  /* ---------- yardımcılar ---------- */
  function esc(s){return (''+(s==null?'':s)).replace(/[&<>"]/g,function(c){return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c];});}
  /* GÜVENLİK: URL şema allowlist — javascript:/data:/vbscript: engellenir (XSS).
     Yalnız http(s), mailto, tel, protokolsüz-göreli ve // kabul edilir. */
  function _safeUrl(u){
    u=(''+(u||'')).trim().replace(/[\u0000-\u001f\u007f]/g,'');
    if(/^(https?:|mailto:|tel:|\/\/)/i.test(u))return u;
    if(/^[a-z][a-z0-9+.-]*:/i.test(u))return '#';   /* bilinmeyen/tehlikeli şema */
    return u||'#';
  }
  /* GÜVENLİK: dış kaynaklı (ProX feed) blok metinleri için hafif sanitizer —
     yalnız b/i/em/strong/br ve şeması doğrulanmış <a href> kalır, gerisi kaçırılır. */
  function _saniInline(h){
    h=''+(h||'');
    return h.replace(/<(\/?)([a-z0-9]+)((?:[^<>"']|"[^"]*"|'[^']*')*)>/gi,function(m0,close,tag,attrs){
      tag=tag.toLowerCase();
      if(['b','i','em','strong','br'].indexOf(tag)>=0)return '<'+close+tag+'>';
      if(tag==='a'){
        if(close)return '</a>';
        var hm=/href\s*=\s*("([^"]*)"|'([^']*)')/i.exec(attrs||'');
        var href=_safeUrl(hm?(hm[2]!=null?hm[2]:hm[3]):'#');
        return '<a href="'+href.replace(/"/g,'&quot;')+'" target="_blank" rel="noopener">';
      }
      return m0.replace(/</g,'&lt;').replace(/>/g,'&gt;');
    });
  }
  ...
  function _stripMd(s){
    s=(''+(s||'')).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');  /* GÜVENLİK: önce kaçır */
    return s.replace(/\*\*(.+?)\*\*/g,'<b>$1</b>').replace(/\[([^\]]+)\]\(([^)]+)\)/g,function(m,t,u){return '<a href="'+_safeUrl(u).replace(/"/g,'&quot;')+'" target="_blank" rel="noopener">'+t+'</a>';});
  }
  ...
  CS.blocksToHtml = function(blocks){
    ...
    return blocks.map(function(b){var d=b.data||{};switch(b.type){
      case 'header': return '<h'+(d.level||2)+'>'+_saniInline(d.text||'')+'</h'+(d.level||2)+'>';
      case 'paragraph': return d.text?('<p>'+_saniInline(d.text)+'</p>'):'';
      case 'quote': return '<blockquote><p>'+_saniInline(d.text||'')+'</p>'+(d.caption?'<cite>'+e(d.caption)+'</cite>':'')+'</blockquote>';
      case 'list': return '<'+(d.style==='ordered'?'ol':'ul')+'>'+(d.items||[]).map(function(i){return '<li>'+_saniInline(typeof i==='string'?i:(i.content||''))+'</li>';}).join('')+'</'+(d.style==='ordered'?'ol':'ul')+'>';
  ...
  CS.mdToHtml = function(md){
    ...
    var inline=function(s){
      s=esc2(s);
      s=s.replace(/!\[([^\]]*)\]\(([^)]+)\)/g,function(m,a,u){return '<img src="'+_safeUrl(u).replace(/"/g,'&quot;')+'" alt="'+a+'" loading="lazy" style="max-width:100%;border-radius:10px;margin:8px 0">';});
      s=s.replace(/\[([^\]]+)\]\(([^)]+)\)/g,function(m,t,u){return '<a href="'+_safeUrl(u).replace(/"/g,'&quot;')+'" target="_blank" rel="noopener">'+t+'</a>';});
      s=s.replace(/\*\*(.+?)\*\*/g,'<b>$1</b>');
      return s;
    };
